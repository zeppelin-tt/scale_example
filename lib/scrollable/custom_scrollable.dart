...
class CustomScrollable extends StatefulWidget {
  ...
  /// ```
  ///
  /// Calling this method will create a dependency on the closest [CustomScrollable]
  /// in the [context], if there is one.
  static CustomScrollableState? of(BuildContext context) {
  ...
  static Future<void> ensureVisible(
    ...
    CustomScrollableState? scrollable = CustomScrollable.of(context);
    while (scrollable != null) {
      ...
      scrollable = CustomScrollable.of(context);
    }
...
class EdgeDraggingAutoScroller {
  ...
  EdgeDraggingAutoScroller(this.scrollable,
      {this.onScrollViewScrolled, this.velocityScalar = _kDefaultAutoScrollVelocityScalar});
  ...
  Future<void> _scroll() async {
    ...
    final Rect globalRect = MatrixUtils.transformRect(scrollRenderBox.getTransformTo(null),
...
class _ScrollableSelectionContainerDelegate extends MultiSelectableSelectionContainerDelegate {
  ...
  Rect _dragTargetFromEvent(SelectionEdgeUpdateEvent event) {
    return Rect.fromCenter(
        center: event.globalPosition, width: _kDefaultDragTargetSize, height: _kDefaultDragTargetSize);
  }
...
/// [ScrollAction] on a [CustomScrollable].
///
/// This is used to configure a [ScrollIncrementDetails] object to pass to a
/// [ScrollIncrementCalculator] function on a [CustomScrollable].
///
...
/// [CustomScrollable.incrementCalculator], or by its defaults if that is not
/// specified.
...
/// An [Action] that scrolls the [CustomScrollable] that encloses the current
...
/// [ScrollAction]s.
///
/// If [CustomScrollable.incrementCalculator] is null for the scrollable, the default
...
class ScrollAction extends Action<ScrollIntent> {
  ...
  bool isEnabled(ScrollIntent intent) {
    ...
    if (contextIsValid) {
      ...
      if (CustomScrollable.of(focus.context!) != null) {
        return true;
      }
  ...
  double _getIncrement(CustomScrollableState state, ScrollIntent intent) {
  ...
  void invoke(ScrollIntent intent) {
    CustomScrollableState? state = CustomScrollable.of(primaryFocus!.context!);
    if (state == null) {
      ...
      if (primaryScrollController!.position.context.notificationContext == null &&
          CustomScrollable.of(primaryScrollController.position.context.notificationContext!) == null) {
        return;
      }
      state = CustomScrollable.of(primaryScrollController.position.context.notificationContext!);
    }
